...
export default function Topbar({ state, dispatch, role, onMenu }) {
  ...
  const contextLabel =
    role === "instructor"
      ...
      : lang === "ar"
        ? `${state.courseId ?? course.id} · الأسبوع ${state.courseWeek ?? course.week}`
        : `${state.courseId ?? course.id} · Week ${state.courseWeek ?? course.week}`;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        ...
        >
          {state.dark ? (
            <IconSun size={15} color={tokens.textMuted} />
          ) : (
            <IconMoon size={15} color={tokens.textMuted} />
          )}
        </button>
        <button
          onClick={() => {
