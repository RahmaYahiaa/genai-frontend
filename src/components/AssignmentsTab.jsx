      ...
      {assignments.length === 0 ? (
        <Card tokens={tokens}>
          <div style={{ textAlign: "center", padding: "30px 16px" }}>
            ...
            <div style={{ fontFamily: hFont, fontWeight: 600, fontSize: 14.5, color: tokens.textPrimary, marginBottom: 5 }}>
              {lang === "ar" ? "لا تكليفات في هذا المقرر بعد" : "No assignments in this course yet"}
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: mobile ? 12 : 16 }}>
          {assignments.map((a) => {
            ...
            return (
              <Card tokens={tokens} key={a.id} style={{ padding: mobile ? "14px 16px" : "16px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, flexWrap: "wrap", flexDirection: isRtl ? "row-reverse" : "row" }}>
                  <div style={{ textAlign: isRtl ? "right" : "left", minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontFamily: bFont, fontSize: 12.5, color: tokens.textMuted, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", flexDirection: isRtl ? "row-reverse" : "row" }}>
                      ...
                      <span>·</span>
                      <span>
                        {topics} {lang === "ar" ? (topics === 1 ? "موضوع" : "مواضيع") : topics === 1 ? "topic" : "topics"}
                      </span>
                      <span>·</span>
                      <span>
                        {a.questions.length} {lang === "ar" ? (a.questions.length === 1 ? "سؤال" : "أسئلة") : a.questions.length === 1 ? "question" : "questions"}
                      </span>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexShrink: 0, flexWrap: "wrap", flexDirection: isRtl ? "row-reverse" : "row", ...(mobile ? { width: "100%" } : {}) }}>
                    {a.status === "draft" && (
                      <Btn
                        tokens={tokens}
                        lang={lang}
                        style={{ padding: "8px 16px", fontSize: 12.5, ...(mobile ? { flex: 1 } : {}) }}
                        onClick={() => {
                          setAssignmentStatus(a.id, "open");
                          toast(lang === "ar" ? `نُشر «${a.title.ar}» — الحالة: مفتوح.` : `Published "${a.title.en}" — status Open.`);
                        }}
                      >
                        {lang === "ar" ? "نشر" : "Publish"}
                      </Btn>
                    ...
                    {open && (
                      <Btn
                        ...
                        onClick={() => {
                          ...
                          toast(lang === "ar" ? `أُغلق «${a.title.ar}» — التسليم معطّل وسجل المراجعة باقٍ.` : `Closed "${a.title.en}" — submission disabled, review history kept.`);
                        }}
                      >
                        {lang === "ar" ? "إغلاق" : "Close"}
                      </Btn>
